...
# Dart Set-up
1. `dart pub get` to get all the dependcies. 
2.  Dart code is here `lib/main.dart`
...
1.  Pull down base image: 
    ```docker pull arm64v8/dart```

2. Build the Docker image: ```docker build -t dart-sdk-arm64 -f compile.Dockerfile .```    

3. Run the Docker container: ```docker run --name dart_container dart-sdk-arm64```

4. Copy the compiled code from the Docker container to your host machine: 

```docker cp dart_container:/app/bootstrap ./lambda_runtime/bootstrap```

### Local Build of Docker Lambda Function and Run

1. ```docker build -t dart-runtime -f ./lambda_runtime/runtime.Dockerfile .``
2. ```docker run dart-runtime```


### Local Development 


dart compile exe ./lib/main.dart -o ./lambda_runtime/bootstrap
chmod +x ./lambda_runtime/bootstrap
zip -r lambda_runtime.zip lambda_runtime


Create the Docker Repo if Not Already exist
-----
aws ecr create-repository --profile summaryze --region 'us-east-1' --repository-name dart-runtime
aws ecr get-login-password --profile summaryze --region 'us-east-1' | docker login --username AWS --password-stdin 479835755234.dkr.ecr.us-east-1.amazonaws.com


Always Build and Push
-----
docker build -t dart-runtime -f ./lambda_runtime/runtime.Dockerfile .
docker tag dart-runtime:latest 479835755234.dkr.ecr.us-east-1.amazonaws.com/dart-runtime:latest
docker push 479835755234.dkr.ecr.us-east-1.amazonaws.com/dart-runtime:latest

Deploying
- Always run both of these
---
sam build --use-container --profile summaryze
sam deploy --guided --profile summaryze
...
```bash
sam delete --stack-name dart_lambda
```
